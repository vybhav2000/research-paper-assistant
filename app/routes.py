...
from app.repositories import clear_conversation_messages, get_paper_or_404, list_papers, save_highlight, serialize_paper_detail
...
from app.services.chat import build_chat_context, build_highlight, generate_answer, retrieve_relevant_chunks, save_chat_exchange
...
def api_chat(paper_id: str, payload: ChatRequest) -> JSONResponse:
    ...
    cited_chunk_ids = [
        ...
        if any(chunk["id"] == chunk_id for chunk in relevant_chunks)
    ]
    save_chat_exchange(
        paper_id,
        message,
...
@router.post("/papers/{paper_id}/highlights", tags=["highlights"], summary="Save a highlight and note")
def api_save_highlight(paper_id: str, payload: HighlightRequest) -> dict:
    ...
    if not payload.chunk_ids or not payload.quote.strip() or not payload.label.strip():
        raise HTTPException(status_code=400, detail="Highlight label, quote, and chunk ids are required.")
